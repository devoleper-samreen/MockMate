    ...
    return (
        <div className="min-h-[90vh] bg-gray-900 text-white flex flex-col items-center px-10">
            ...
            <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl w-full">
                <SelectionCard icon={<FaUserTie />} status='Coming Soon...' title="Become an Interviewer" description="Host an interview and help others prepare." onClick={() => navigate("/become-interviewer")} />
                <SelectionCard icon={<FaCalendarAlt />} status='Coming Soon...' title="Schedule an Interview" description="Plan your interview with an interviewer." onClick={() => navigate("/schedule-interview")} />
                <SelectionCard icon={<FaRocket />} status='Live' title="Instant Interview" description="Start an interview now. We can connect you with a peer or interviewer." onClick={handleInstantInterview} />
            </div>
...
const SelectionCard = ({ icon, title, description, onClick, status }) => (
    <div className="bg-gray-800 p-8 rounded-lg shadow-lg text-center cursor-pointer hover:scale-105 transition-transform" onClick={onClick}>
        <div className="flex text-4xl mb-6 text-yellow-400 items-center justify-between">
            {icon}
